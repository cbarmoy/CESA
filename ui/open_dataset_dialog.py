"""Unified dialog to select an EDF file and choose the loading mode."""
...
class OpenDatasetDialog:
    ...
    def _build_ui(self) -> None:
        ...
        # EDF selection -------------------------------------------------
        edf_group = ttk.LabelFrame(container, text="Fichier EDF")
        ...
        info_label = ttk.Label(
            edf_group,
            text="Choisissez le fichier EDF à analyser.",
            foreground="#555555",
        )
    ...
    def _browse_edf(self) -> None:
        path = filedialog.askopenfilename(
            parent=self.window,
            title="Sélectionner un fichier EDF",
            filetypes=[("Fichiers EDF", "*.edf"), ("Tous les fichiers", "*.*")],
        )
        if path:
    ...
    def _confirm(self) -> None:
        ...
        if not edf_path:
            messagebox.showwarning("Fichier manquant", "Veuillez sélectionner un fichier EDF.", parent=self.window)
            return
        if not Path(edf_path).exists():
            messagebox.showwarning(
                "Fichier introuvable",
                "Le fichier EDF sélectionné n'existe pas.",
                parent=self.window,
            )
            return
