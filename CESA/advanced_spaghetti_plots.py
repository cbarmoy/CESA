...
def create_spaghetti_plot(
        ...
        if len(subjects) <= 20:
            if normalized_clusters:
                ...
                leg = ax.legend(handles=handles, title='Legend', loc='center left', bbox_to_anchor=(1.02, 0.5), 
                               frameon=True, fontsize=12, borderaxespad=0)
            else:
                handles = []
                # First add mean lines if present
                if mean_lines_legend:
    ...
    ax_table.set_title('Robust intra-subject metrics — codes: UP/DN/=, * = significant', fontsize=13, fontweight='bold', pad=20)
...
def _edf_to_metrics(
    ...
    for psg_path in edf_files:
        ...
        try:
            ...
            raw = open_raw_file(psg_path, preload=False, verbose="ERROR")  # type: ignore
        except Exception:
            try:
