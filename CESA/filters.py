...
def apply_filter(
    ...
    *,
    ...
    """Apply IIR Butterworth filter to a 1D signal array.
    ...
    ----------
    data : np.ndarray
        The input 1D signal in microvolts (µV).
    sfreq : float
    ...
    """
    try:
        # Local import to keep hard dependency optional at module import time
        from scipy.signal import butter, filtfilt
    except Exception:
        # If SciPy is missing, return input unchanged to keep UI responsive
        return data

    try:
        if not isinstance(data, np.ndarray):
            data = np.asarray(data, dtype=float)
        # Sanitize to avoid propagation of NaN/Inf which can cause straight lines
        ...
        if low > 0.0 and high > 0.0:
            ...
            b, a = butter(int(filter_order), wn, btype="band")
        elif low > 0.0 and high == 0.0:
            # High-pass
            if not (0.0 < low < nyquist):
                return data
            wn = low / nyquist
            b, a = butter(int(filter_order), wn, btype="highpass")
        elif (low == 0.0) and high > 0.0:
            # Low-pass
            if not (0.0 < high < nyquist):
                return data
            wn = high / nyquist
            b, a = butter(int(filter_order), wn, btype="lowpass")
        else:
            # No filtering
            return data
        ...
        # In case of numerical issues, return input unchanged
        return data
