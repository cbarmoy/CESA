...
class PSGPlotter:
    ...
    filter_order : int
        Ordre du filtre Butterworth (provient du profil actif, défaut 4).
    filter_type : str
        Type de filtre IIR: "butterworth", "cheby1", "cheby2", "ellip".
    theme_name : str | None
    ...
    def __init__(
        ...
        *,
        ...
        signal_sections: Optional[List[Dict[str, Any]]] = None,
        channel_section_map: Optional[Dict[str, str]] = None,
        ...
        self.signal_sections: List[Dict[str, Any]] = signal_sections or [
            {"key": "eeg", "label": "EEG", "signal_type": "eeg", "ratio": 5.0, "enabled": True},
            {"key": "eog", "label": "EOG", "signal_type": "eog", "ratio": 1.2, "enabled": True},
            {"key": "emg", "label": "EMG", "signal_type": "emg", "ratio": 1.2, "enabled": True},
            {"key": "ecg", "label": "ECG", "signal_type": "ecg", "ratio": 1.2, "enabled": True},
        ]
        self.channel_section_map: Dict[str, str] = dict(channel_section_map or {})
        self.hypnogram_ratio = float(max(0.2, hypnogram_ratio))
        self.events_ratio = float(max(0.2, events_ratio))
        ...
        self._line_store: Dict[str, Dict[str, Any]] = {}
        self._legend_done: Dict[str, bool] = {}
    ...
    def _build_figure(self) -> Figure:
        ...
        self.figure = fig
        active_sections = [s for s in self.signal_sections if bool(s.get("enabled", True))]
        if not active_sections:
            active_sections = [{"key": "eeg", "label": "EEG", "signal_type": "eeg", "ratio": 5.0, "enabled": True}]
        self._active_sections = active_sections

        ratios = [self.hypnogram_ratio] + [float(max(0.2, s.get("ratio", 1.0))) for s in active_sections] + [self.events_ratio]
        gs = fig.add_gridspec(len(ratios), 1, height_ratios=ratios)
        self.ax_hypno = fig.add_subplot(gs[0])

        self._signal_axes_by_key: Dict[str, Any] = {}
        self._signal_keys_order: List[str] = []
        share_axis = None
        row_idx = 1
        for section in active_sections:
            key = str(section.get("key", "")).strip() or f"section_{row_idx}"
            ax = fig.add_subplot(gs[row_idx], sharex=share_axis)
            if share_axis is None:
                share_axis = ax
            self._signal_axes_by_key[key] = ax
            self._signal_keys_order.append(key)
            row_idx += 1

        self.ax_events = fig.add_subplot(gs[row_idx], sharex=share_axis)
        self.axes_all: List[Any] = [self.ax_hypno] + [self._signal_axes_by_key[k] for k in self._signal_keys_order] + [self.ax_events]
    ...
    def _redraw(self, draw_only: bool = False) -> None:
        ...
        # Signals by configured sections
        for section in self._active_sections:
            key = str(section.get("key", "")).strip()
            if key and key in self._signal_axes_by_key:
                self._plot_signals_of_type(self._signal_axes_by_key[key], target_type=key)
        ...
        xlims_changed = False
        signal_axes = [self._signal_axes_by_key[k] for k in self._signal_keys_order]
        for ax in signal_axes + [self.ax_events]:
        ...
        try:
            for ax in [self.ax_hypno] + signal_axes:
                ax.set_xlabel("")
    ...
    def _plot_signals_of_type(self, ax, *, target_type: str) -> None:
        ...
        colors = self._channel_colors(channel_names, target_type=target_type)
        xs: List[np.ndarray] = []
        ys: List[np.ndarray] = []
        used_names: List[str] = []
        ...
        if plotted == 0 and len(channel_names) == 0:
            ...
            section_meta = self._section_meta_by_key(target_type)
            spacing_type = str(section_meta.get("signal_type", target_type)).lower()
            if spacing_type == "eeg":
                spacing = max(40.0, base * 1.2)
            elif spacing_type in ("eog", "emg"):
                spacing = max(25.0, base * 1.2)
            else:
                spacing = max(20.0, base * 1.2)

            # Reuse or create lines; update with set_data for speed
            store = self._line_store.setdefault(target_type, {})
            ...
            try:
                if self._use_blit and self.figure.canvas is not None and not self._blit_ready:
                    self.figure.canvas.draw()
                    for a in [self._signal_axes_by_key[k] for k in self._signal_keys_order] + [self.ax_events]:
    ...
    def _select_channels_for_type(self, target_type: str) -> List[str]:
        # New behaviour: explicit profile mapping (target_type is section key).
        mapped = [
            ch for ch, section_key in self.channel_section_map.items()
            if section_key == target_type and ch in self.signals
        ]
        if mapped:
            return mapped

        # Legacy behaviour: target_type is one of eeg/eog/emg/ecg.
        section_meta = self._section_meta_by_key(target_type)
        effective_type = str(section_meta.get("signal_type", target_type)).lower()
        available = [ch for ch in self.signals.keys() if cesa_detect_signal_type(ch) == effective_type]
        if effective_type == "eeg":
    ...
    def _section_meta_by_key(self, key: str) -> Dict[str, Any]:
        for section in getattr(self, "_active_sections", []):
            if str(section.get("key", "")).strip() == key:
                return section
        return {}

    def _channel_colors(self, channel_names: List[str], *, target_type: str = "") -> List[str]:
        ...
        palette = [
            str(c).strip().upper()
            for c in (section_meta.get("color_palette", []) or [])
            if isinstance(c, str) and str(c).strip()
        ]
        if palette:
            # Force visible contrast even with short or close palettes.
            # This keeps "palette per section" readable for many channels.
            contrast_cycle = [0.75, 1.0, 1.25, 0.9, 1.1]
            varied: List[str] = []
            for i, _ in enumerate(channel_names):
                base = palette[i % len(palette)]
                factor = contrast_cycle[(i // max(1, len(palette))) % len(contrast_cycle)]
                varied.append(_shade(base, factor))
            return varied
