...
class LazyProvider(SignalProvider):
    ...
    def _resolve_raw(source: str | Path | BaseRaw) -> BaseRaw:
        ...
        if mne is None:  # pragma: no cover - defensive
            raise RuntimeError("mne is required to load raw PSG files")
        suffix = path.suffix.lower()
        if suffix in {".edf", ".bdf"}:
            return mne.io.read_raw_edf(str(path), preload=False, verbose=False)
        if suffix == ".fif":
            return mne.io.read_raw_fif(str(path), preload=False, verbose=False)
        raise ValueError(f"Unsupported raw format for LazyProvider: {path.suffix}")
